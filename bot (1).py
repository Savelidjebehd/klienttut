...
async def ensure_telethon() -> TelegramClient:
    """
    Возвращает подключённый Telethon-клиент в режиме ПОЛЬЗОВАТЕЛЯ.
    SESSION_STRING получен через get_session.py — авторизация не нужна при каждом запуске.
    """
    global telethon_client
    if telethon_client is None or not telethon_client.is_connected():
        if not SESSION_STRING:
            raise RuntimeError(
                "SESSION_STRING пустой! Запустите get_session.py и вставьте строку."
            )
        telethon_client = TelegramClient(StringSession(SESSION_STRING), API_ID, API_HASH)
        await telethon_client.connect()   # connect() без start() — сессия уже есть
